...
export abstract class BaseRepository<T, ID> implements IRepository<T, ID> {
  ...
  async findById(id: ID): Promise<T | null> {
    const sql = `SELECT * FROM "${this.tableName}" WHERE "${this.idField}" = $1 LIMIT 1`;
  ...
  async update(id: ID, entity: Partial<T>): Promise<T | null> {
    ...
    const sql = `UPDATE "${this.tableName}" SET ${setClauses} WHERE "${this.idField}" = $${values.length + 1} RETURNING *`;
    return this.dataGateway.getOne<T>(sql, [...values, id]);
  }
